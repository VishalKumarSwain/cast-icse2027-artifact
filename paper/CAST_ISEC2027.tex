...
\begin{itemize}
\item \textbf{CAST}, a controlled, distance-instrumented diagnostic methodology for evaluating the attribution stability of already-trained AI-code detectors under routine, non-adversarial edits. CAST's canonicalization intervention and identity-transformation noise-floor control exist specifically to prevent the two failure modes this paper's own results expose: an apparent stability finding confounded by degenerate baseline accuracy, and one confounded by re-scoring noise. A low Induced Error Rate is uninterpretable without both checks, and this paper demonstrates why on two of its own three evaluated detectors, not as a separate lesson but as CAST's methodology doing its job.
\item An empirical result: a reconstructed but accuracy-validated detector (DroidDetect-Base) shows a large, reproducible, class-specific, and directionally asymmetric instability under reformatting --- flipping correct human-code verdicts to ``AI'' far more often than the reverse (Fisher's exact $p{<}10^{-11}$) --- consistent with the detector having learned tidy formatting as an AI-authorship proxy.
\end{itemize}
...
\caption{The CAST framework. Top: the main pipeline -- a transformation $T$ is applied to a program $C_0$ of known provenance (1), validated (2) and distance-measured (3) against $C_0$, then both $C_0$ and $T(C_0)$ are scored by all three detectors (4) to compute score movement and the Induced Error Rate (5). Bottom-left: when movement is large, the canonicalization intervention scores a presentation-stripped version of both sides to isolate presentation-driven instability. Bottom-right: the identity-transformation noise-floor control re-scores $C_0$ against itself with a fresh seed and no transformation, establishing how much decision-flip rate is measurement noise rather than a real transformation effect.}
\label{fig:cast}
\end{figure*}
...
\textbf{DroidDetect-Base} (ModernBERT-base~\cite{modernbert} with a projection layer and a four-class classification head) ships a public checkpoint with no accompanying modeling code. We reconstructed its architecture from the checkpoint's own parameter names and shapes, and validated the reconstruction against a disjoint dev-split subset ($n{=}24$), reaching 87.5\% accuracy (Wilson 95\% CI [69.0, 95.7]). This interval does not fully contain the 98--99\% accuracy the same reconstruction reaches on the held-out test split used throughout the rest of this study (Section~4.1). We cannot yet fully rule out that this reflects a genuine reconstruction gap rather than dev-subset difficulty, and treat this as an open validation question (Section~6); we note, though, that a fundamentally incorrect reconstruction of a 4-class head would be expected to perform near the 25\% chance level, not 87.5\%, making a substantially correct implementation more consistent with the dev-split result than a severely broken one; $n{=}24$ also genuinely limits precision here (each additional correct call shifts accuracy by 4.2 percentage points), so the interval's coarseness reflects sample size as much as it reflects disagreement. Because this paper's central empirical finding rests entirely on this one reconstructed detector, this is the single most important threat to the paper's soundness, and future work (Section~8) prioritizes closing it directly.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.75\textwidth]{figures/figure3_canonicalization_intervention.png}
