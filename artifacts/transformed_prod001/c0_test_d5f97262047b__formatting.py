...
def send_cog_version(bot, msg):
    cog_names = [
        ...
        if word.lower() not in ("help", "version")
    ]
    ...
    cogs = bot.get_cog(cog_names[0]) if cog_names else []

    response_to_send = ""
    if not cogs and cog_names:
    ...
    for cog in cogs:
        response_to_send += (
            f"{cog.qualified_name}'s version: {cog.version}\n"
            if hasattr(cog, "version")
            else "No version available for this cog.\n"
        )

    if cog_names and response_to_send:
        ...
        await send(
            msg.channel,
            "Multiple cog names provided. Not all may have a version. Check if you got them right.",
        )
...
@bot.command()
async def version(ctx, *, cog_names):
