def forward(self, emb_in, length, state_init=None, batch_size=1, mask=None):
    ...
    state_init = tensor.alloc(numpy.float32(0.0), batch_size, self.dim)

    # calculate the input vector for inputter, updater and reseter
    state_in = (tensor.dot(emb_in, self.input_emb) + self.input_emb_offset).reshape(
    ...
    hiddens, updates = theano.scan(
        scan_func, sequences=scan_inp, outputs_info=outputs_info
    )
    if self.verbose:
        return (
            hiddens[0],
