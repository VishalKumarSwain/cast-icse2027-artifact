public class AntsOperator_Sobel_Thin extends AtsProcessOperator {
  ...
  protected void convolution(
    ...
    float[] kernel = new float[] {-1, 0, 1, -2, -2, 4, -1, 0, 1};

    int[] sumX = {0, 0, 0};

    int[] sumY = {0, 0, 0};

    AtsutyUtils.convolution(pixelFormat, argb, width, height, 3, kernel, sumX, sumY);
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        float length_ = (length <= 0.1f) ? 0f : Math.max(length * scale - bias, 0f);
