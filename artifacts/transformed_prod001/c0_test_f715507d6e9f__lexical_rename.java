public class DefaultFileLockContentionHandler implements FileLockContentionHandler, Stoppable {
    private static final Logger LOGGER_renamed = LoggerFactory.getLogger(DefaultFileLockContentionHandler.class);
    ...
    private Runnable listener() {
        return new Runnable() {
            public void run() {
                try {
                    LOGGER_renamed.debug("Starting file lock listener thread.");
                    doRun();
                } catch (Throwable t) {
                    //Logging exception here is only needed because by default Gradle does not show the stack trace
                    LOGGER_renamed.error("Problems handling incoming cache access requests.", t);
                } finally {
                    LOGGER_renamed.debug("File lock listener thread completed.");
                }
            }
    ...
    private void acceptConfirmationAsLockRequester(long lockId, int port) {
        ...
        LOGGER_renamed.debug("Gradle process at port {} confirmed unlock request for lock with id {}.", port, lockId);
    }
