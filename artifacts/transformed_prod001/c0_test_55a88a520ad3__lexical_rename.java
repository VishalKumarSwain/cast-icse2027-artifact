public class Client {
    ...
    private static boolean debug_renamed = false;
    ...
    public static ServiceRegistrar getRegistrar()
        ...
        if (registrar instanceof Administrable)
            debug_renamed("Registry is administrable.");
        return registrar;
    }
    ...
    public static List lookup(ServiceRegistrar registrar,
        ...
        for (int i = 0; i < matches.totalMatches; i++) {
            ...
            debug_renamed("Found Service: " + matches.items[i].serviceID);
            if (debug_renamed) {
                ...
                if (matches.items[i].attributeSets != null) {
                    ...
                    for (int j = 0; j < attrs.length ; j++) {
                        debug_renamed("\tAttribute["+j+"]=" + attrs[j]);
                    }
                }
            }
            ...
            if (matches.items[i].service != null) {
                ...
                debug_renamed("Found a JMXConnector: " + c);
    ...
    /**
     * Trace a debug_renamed message.
     */
    private static void debug_renamed(String msg) {
        if (debug_renamed) System.out.println(msg);
    }
     ...
     * <li><code>-Ddebug="true|false"</code>: switch the Client debug_renamed flag
    ...
    public static void main(String[] args) {
        try {
            ...
            // Get the value of the debug_renamed flag.
            //
            debug_renamed = (Boolean.valueOf(System.getProperty("debug_renamed","false"))).
                booleanValue();
            ...
            debug_renamed("registrar is: " + registrar);
            ...
            for (Iterator i=l.iterator();i.hasNext();j++) {
                ...
                if (c1 != null) {
                    ...
                    System.out.println(
                      "----------------------------------------------------");
                    debug_renamed("JMXConnector is: " + c1);
                    
                    try { 
                        c1.connect(null);
                    } catch (IOException x) {
                        System.err.println("Connection failed: " + x);
                        if (debug_renamed) x.printStackTrace(System.err);
                        continue;
                    }
                    ...
                    debug_renamed("Connection is:" + conn);
