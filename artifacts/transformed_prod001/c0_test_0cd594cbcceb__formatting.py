def build_autoscale_profile(autoscale_settings):
    ...
    def _validate_default_profile(default_profile, profile):
        if (
            profile.capacity.default != default_profile.capacity.default
            or profile.capacity.minimum != default_profile.capacity.minimum
            or profile.capacity.maximum != default_profile.capacity.maximum
        ):
    ...
    default_profiles = [
        x for x in autoscale_settings.profiles if not x.recurrence and not x.fixed_date
    ]
    profile_schedule = {}
    ...
    for profile in recurring_profiles:
        try:
            ...
            end_time = time(
                hour=profile.recurrence.schedule.hours[0],
                minute=profile.recurrence.schedule.minutes[0],
            )
            if not default_profile:
                default_profile = AutoscaleProfile(
                    name="default", capacity=profile.capacity, rules=profile.rules
                )
            else:
