public class StageManager {
  ...
  protected static void initialize(Stage stage) throws IOException {
    ...
    StageManager.icon =
        new Image(
            StageManager.class.getClassLoader().getResourceAsStream("icons/logo_transparent.png"));
    StageManager.viewToScene = new EnumMap<>(View.class);

    // Load login elements prior to initializing loader.
    FXMLLoader loginLoader =
        new FXMLLoader(
            StageManager.class.getClassLoader().getResource(View.LOGIN_VIEW.getPathToFXML()));
  ...
  /**
   * Sets the FXML node to be displayed on screen, specified by {@code view}. Closes the previous
   * stage and creates a new one.
   */
  public static void loadView(View view) {
    ...
    if (controller instanceof Refreshable) ((Refreshable) controller).refresh();
    else
      System.err.printf(
          "StageManager can only handle controllers of type 'Refreshable', missing in '%s'.",
  ...
  /**
   * Returns the {@link Scene} object associated with given {@link View}. If no scene is present, a
   * new one is created.
   */
