...
def extract_data_from_s3(bucket_name, file_key):
    ...
    s3 = boto3.client("s3")
    try:
        s3.download_file(bucket_name, file_key, "local_data.csv")
...
def extract_data_from_gcs(bucket_name, file_key):
    print(
        f"Starting extraction from Google Cloud Storage: bucket={bucket_name}, key={file_key}"
    )
...
def load_data_to_s3(transformed_data, bucket_name, output_key):
    ...
    try:
        ...
        s3 = boto3.client("s3")
        s3.upload_file("transformed_data.csv", bucket_name, output_key)
...
def load_data_to_gcs(transformed_data, bucket_name, output_key):
    print(
        f"Starting loading to Google Cloud Storage: bucket={bucket_name}, key={output_key}"
    )
    try:
        transformed_data.to_csv("transformed_data.csv", index=False)
        client = storage.Client()
