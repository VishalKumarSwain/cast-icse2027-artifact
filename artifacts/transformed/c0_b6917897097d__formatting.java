...
/**
 * Listener that accepts events generated by case management module. These events are related to
 * case work items.
 *
 * @author mederly
 */
...
public class CaseManagementListener implements CaseWorkItemListener {
  ...
  @Override
  public void onWorkItemCreation(
      CaseWorkItemType workItem, CaseType aCase, Task task, OperationResult result) {
    for (ObjectReferenceType assigneeRef : workItem.getAssigneeRef()) {
      CaseWorkItemEvent event =
          new CaseWorkItemEvent(
              identifierGenerator,
              ChangeType.ADD,
              workItem,
              SimpleObjectRefImpl.create(functions, assigneeRef),
              aCase);
  ...
  private void processEvent(CaseWorkItemEvent event, OperationResult result) {
    try {
      ...
      result.recordFatalError(
          "An unexpected exception occurred when preparing and sending notifications: "
              + e.getMessage(),
          e);
