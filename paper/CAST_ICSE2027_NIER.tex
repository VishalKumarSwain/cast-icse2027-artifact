...
Table~\ref{tab:ier} reports IER with baseline-correct denominators. DroidDetect-Base's formatting IER exceeds its lexical-rename and control-flow IER in both languages, which collapse to at or near 0\% in Python. Wilson~\cite{wilson1927} 95\% CIs for formatting do not overlap the other families in Python; Java's smaller post-collapse denominators ($n{=}16$--$82$) give wider, overlapping intervals, so the Java effect is directionally consistent but less sharply separated. LLMSniffer's IER stays below 8.3\% throughout. DetectCodeGPT's IER is elevated broadly (16.7--50.0\%), peaking under control-flow rather than formatting -- but an identity control (Section~VI) found a comparably sized 30.0\% flip rate on \emph{unchanged} input, so this elevated IER cannot yet be distinguished from re-scoring noise. Median absolute score movement is consistent: little except under formatting for DroidDetect-Base (median 0.0055, up to 0.14); uniformly small for LLMSniffer (median 0.0106); broad regardless of family for DetectCodeGPT (median 0.478).
...
Local structural extraction and semantics-preserving rewriting were additionally attempted at two intensities. Of 796 attempted (language, family, intensity) combinations, only 37 (4.65\%) were applicable, of which 35 validated successfully; only 4 of those 35 received true differential-execution verification, and the remaining 31 degraded to structural-only validation for lack of an unambiguous, stdin-free entry point. This reflects Droid's composition: a substantial share of its rows are function- or class-level fragments rather than complete executable programs, bounding how densely this evaluation style currently extends to deeper transformations. The resulting per-detector cells ($n{=}1$--$12$) are illustrative only and support no confirmatory claim.

\section{Discussion}

Static accuracy does not characterize post-transformation reliability: detectors with non-trivial numbers of correct baseline calls (Table~\ref{tab:ier}'s $n$ column) diverge sharply after a routine edit, though a low per-class baseline accuracy is itself a threat to how an apparently ``stable'' IER should be read (Section~VI). The two unstable detectors' failure modes differ qualitatively: DroidDetect-Base's instability is narrow, presentation-driven, and reproducible (deterministic inference); DetectCodeGPT's is broad and not yet separable from its own re-scoring noise -- itself a reliability finding, since a verdict that depends on a random seed is unfit for the single-verdict use this paper's motivation describes. This motivates reporting stability under transformation alongside accuracy, and canonicalization as a lightweight diagnostic that, for presentation-sensitive detectors, is also a candidate mitigation. CAST does not identify \emph{why} a detector is presentation-sensitive or diffusely unstable; that requires inspecting internal representations or training data, outside this paper's scope.
...
\item \textbf{Construct:} Detector scores have incompatible scales (Section~II-B), and DetectCodeGPT's weak calibration (Section~III) limits its threshold-derived results to secondary status. $d_\text{text}$, $d_\text{token}$, and $d_\text{AST}$ are complementary but incomplete edit-magnitude proxies, not yet related to IER/$|\Delta|$ directly. DetectCodeGPT's reported IER and $|\Delta|$ (Section~IV-A) are upper bounds that include an identity-control noise floor (30.0\% flip rate, mean $|\Delta|{=}0.925$) of comparable size; LLMSniffer and DroidDetect-Base use no sampling at inference and have a zero noise floor by construction.
\item \textbf{Internal:} DroidDetect-Base's architecture was reconstructed from its checkpoint absent public modeling code. Its IER (Table~\ref{tab:ier}) uses the human-versus-AI collapse of its reconstructed four-class output, not raw four-class flips; of 365 transformed pairs, 16 flips were intra-AI churn (MACHINE\_GENERATED$\leftrightarrow$MACHINE\_REFINED) excluded on this basis. The canonicalizer normalizes several presentation properties at once, so a positive result supports a claim about that combined category, not which property is responsible; detector accuracy on canonicalized inputs is not yet reported.
...
This evaluation will be extended in three directions. (1) Extending the identity-noise-floor control to the full 199-program test set, reporting IER net of noise rather than as an upper bound. (2) A corpus of complete, executable programs with chained, multi-step transformations mined from real commit histories, scaling beyond the 4.65\% applicability of function-level fragments (Section~IV-D). (3) A component ablation of canonicalization and an analysis relating IER and $|\Delta|$ to $d_\text{text}$, $d_\text{token}$, $d_\text{AST}$, alongside a detector trained against CAST's transformation families to test whether stability can be achieved by construction.

\section{Conclusion}

This paper presented CAST, a controlled, distance-instrumented methodology for testing attribution stability, applied to three reproducible detectors over 199 held-out Python and Java programs. The detectors show qualitatively different profiles: one shows formatting-specific sensitivity substantially attenuated by canonicalization; a second stays stable throughout; a third shows broad movement not yet separable from re-scoring noise.
