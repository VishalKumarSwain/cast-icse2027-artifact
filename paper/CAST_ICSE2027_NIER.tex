...
Table~\ref{tab:ier} reports IER with baseline-correct denominators. DroidDetect-Base's formatting IER exceeds its lexical-rename and control-flow IER in both languages, collapsing to at or near 0\% in Python; Wilson~\cite{wilson1927} 95\% CIs for formatting do not overlap the other families in Python, while Java's smaller denominators give wider, overlapping intervals. Table~\ref{tab:byclass} breaks this down by true origin class: DroidDetect-Base's formatting IER is 58.0\% on genuinely human-written code but 0.0--4.3\% on actually AI-authored code, in both languages separately (Python 66.7\%/0.0\%/6.5\%; Java 41.2\%/0.0\%/0.0\%). This is not a no-op artifact (only 5--6\% of AI-class transformations are byte-identical); nor is it explained by edit size -- flipped human samples have \emph{smaller} mean $d_\text{text}$ (81.1) than unflipped ones (470.5). $|\Delta|$ correlates weakly with $d_\text{text}$ ($\rho{\le}0.30$) but not $d_\text{AST}$ ($\rho{=}{-}0.20$, $0.03$), consistent with a presentation- not structure-driven effect. LLMSniffer's low IER is not robustness evidence (67.0\%/44.4\% baseline accuracy, at/below majority; Section~VI); DetectCodeGPT's elevated IER (16.7--50.0\%) sits within its 30.0\% identity-control noise floor (Section~VI).

\textbf{Answer:} DroidDetect-Base's instability is real, class-specific, and overwhelmingly false-accusing; the other two detectors' apparent profiles are confounded by degenerate baseline accuracy and re-scoring noise, respectively.
...
Static accuracy does not characterize post-transformation reliability, but neither does a low IER by itself characterize stability: LLMSniffer's near-majority-class accuracy makes its low IER a likely degenerate-predictor artifact, and DetectCodeGPT's IER is not yet separated from re-scoring noise. Only DroidDetect-Base's instability survives both checks -- a real detector (98--99\% baseline accuracy) whose formatting sensitivity is reproducible and overwhelmingly false-accusing. A verdict that depends on a random seed, or on a detector no better than guessing the majority class, is unfit for single-verdict use. Table~\ref{tab:byclass} rules out the simplest confound -- formatting as a near-no-op on AI code -- since flipped human samples have \emph{smaller} edits than unflipped ones; a detector that learned tidy formatting as an AI-authorship proxy fits better than edit magnitude, though CAST does not identify the mechanism directly. Canonicalization is a diagnostic, not a validated mitigation: its own out-of-distribution effect means a practical fix needs a layout-preserving normalizer, not this token-rejoining canonicalizer.
...
\item \textbf{Construct:} LLMSniffer's baseline accuracy (67.0\%/44.4\%, at/below majority) means its low IER is not robustness evidence. Detector scores have incompatible scales, and DetectCodeGPT's weak calibration limits threshold-derived results to secondary status; its IER/$|\Delta|$ are upper bounds including a comparably sized identity-control noise floor. $d_\text{text}$, $d_\text{token}$, $d_\text{AST}$ correlate only weakly with $|\Delta|$.
\item \textbf{Internal:} DroidDetect-Base's architecture was reconstructed absent public modeling code, not yet compared against the original authors' numbers. Its IER uses the human-versus-AI collapse of its four-class output; 16 of 365 flips were intra-AI churn, excluded on this basis. The canonicalizer normalizes several presentation properties at once, so a result is about that combined category, not which is responsible; canonicalized-input accuracy shows a real out-of-distribution effect for DroidDetect-Base not fully controlled for.
...
Measuring prediction change under fixed, semantics-preserving transformations is established for code models generally: Rabin et al.~\cite{rabin} show neural program models fail to generalize under such edits, and ReCode~\cite{recode} benchmarks code-generation robustness under $>$30 similar operators; CAST applies the same kind of edit to a different task and failure mode -- attribution false-accusation, not degraded generation. Table~\ref{tab:novelty} positions CAST against the closest prior work on whether it edits code post-generation, is routine rather than adversarial, targets already-trained AI-code detectors specifically, and compares detectors under identical edits. Detector papers~\cite{gptsniffer,llmsniffer,detectcodegpt,codegptsensor} report accuracy, occasionally under one perturbation, without stratifying transformations. CodeGPTSensor+~\cite{codegptsensorplus} and Droid~\cite{droid} share the observation that renaming can obscure AI origin, but address it with adversarial training or attack scenarios. Pordanesh et al.~\cite{hidinginplainsight} vary training-set and prompting conditions, not the source code itself. RoPGen~\cite{ropgen} applies style transformation \emph{during training} to harden a \emph{human}-authorship attributor; CAST transforms \emph{at evaluation time}, against \emph{already-trained}, unmodified detectors. Whitespace-based detection work~\cite{whitespaces} motivates why formatting sensitivity can be presentation-driven.
...
This evaluation will be extended in three directions. (1) A symmetric experiment perturbing AI code toward human-like ``messiness'' to test for the reverse flip, directly probing the tidy-formatting-as-AI-proxy hypothesis, plus a layout-preserving normalizer as a genuine mitigation candidate. (2) A corpus of complete, executable programs with chained transformations mined from real commit histories, beyond the 4.65\% applicability of function-level fragments. (3) Comparing reconstructed DroidDetect-Base against the original authors' numbers, shared-seed DetectCodeGPT, and a detector trained against CAST's transformations.

\section{Conclusion}

This paper presented CAST, a controlled, distance-instrumented methodology for testing attribution stability. Its clearest finding: a validated detector flips correct verdicts on human-written code to ``AI'' 58.0\% of the time after routine reformatting, versus 0.0--4.3\% on actually AI-authored code -- consistent with tidy formatting as a learned AI-authorship proxy, not edit magnitude. The other two detectors' profiles are confounded by baseline accuracy and re-scoring noise: a low IER alone is not robustness evidence.
