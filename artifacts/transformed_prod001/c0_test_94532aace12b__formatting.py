...
class NeuralNetworkLayer:
    def apply(
        self, input_tensor, weight_blocks, evaluation=None, separate_bn_variables=False
    ):
        ...
        with tf.variable_scope("layer_scope", reuse=tf.AUTO_REUSE):
            ...
            regularizer = self._get_l2_regularizer()
            var_getter = partial(
                custom_var_getter, regularizer=regularizer, evaluation=evaluation
            )

            # Create weights using the custom getter
            weights = [
                var_getter(f"weight_{i}", shape=block.shape)
