...
On 199 held-out Python and Java programs from the Droid corpus, reformatting with a standard tool (\texttt{black}/\texttt{google-java-format}) flips a reconstructed detector's (DroidDetect-Base) correct verdict on human-written code to ``AI'' 58.0\% of the time (29/50), against 2/105 on AI-authored code under the same transformation (Fisher's exact $p{<}10^{-15}$) and 6/119 (5.0\%) under a reverse perturbation. The effect is concentrated in short programs (88\% in the shorter half of human programs, 28\% in the longer half) but persists at matched length, and it is not explained by edit size. The other two detectors show why a low instability rate is not evidence of robustness: LLMSniffer performs at or below a majority-class baseline on this corpus (67.0\% Python, 44.4\% Java), and DetectCodeGPT performs at chance (49.7--50.3\%) and moves as much on \emph{unchanged} input, re-scored under a different perturbation seed, as under formatting, even when both sides share seeds. Canonicalization reduces DroidDetect-Base's residual movement 14.8--15.4$\times$, but the canonicalized input is often out-of-distribution for the detector, so it is not yet a mitigation. Static accuracy alone does not characterize detector reliability under routine code evolution.
\end{abstract}
...
\item An empirical result: a reconstructed detector (DroidDetect-Base, 86.4\% four-class accuracy on the test corpus, consistent with its dev-split check) shows a large, class-specific, directionally asymmetric instability under reformatting: correct human-code verdicts flip to ``AI'' far more often than for AI-class code under the same transformation (Fisher's exact $p{<}10^{-14}$), concentrated in short programs but present at matched length. This is consistent with the detector treating tidy formatting as a proxy for AI authorship.
\end{itemize}
...
\item \textbf{RQ3:} Is canonicalization a valid diagnostic of presentation-driven instability, and does its behavior differ across detectors?
\end{itemize}
...
We test the asymmetry in two ways. The primary contrast uses the same transformation and the same binary baseline filter on both sides: formatting flips 29/50 human-written programs but 2/105 AI-class programs (odds ratio 71, Fisher's exact $p{=}9.6{\times}10^{-16}$). As a check with a different transformation, we messified AI-class code (introducing trailing whitespace, extra blank lines, and inconsistent quoting and spacing: a deterministic, semantics-preserving, validity-checked transformation) on the 130 AI-class programs that DroidDetect-Base calls AI at baseline, of which 119 remained after excluding 11 whose messified output failed validation, and measured the AI-to-human flip rate. This induces a flip in 6/119 cases (5.0\%, 95\% CI [2.3, 10.6]; Python 3/54, Java 3/65), far below the 58.0\% human-to-AI rate (odds ratio 26, Fisher's exact $p{=}1.2{\times}10^{-13}$). Requiring the exact four-class label at baseline instead gives 5/96 (5.2\%). The messify comparison is not intensity-matched: its mean length-normalized edit size ($d_\text{text}/\text{len} = 0.064$) is smaller than formatting's on human code ($\approx$0.14), so it confirms a real, directional asymmetry rather than measuring a matched effect size.
...
The effect is not specific to one source dataset. Human programs flip in 10/16 (The Vault functions), 10/11 (TACO), 6/10 (StarCoder), 2/9 (The Vault classes, median length 4,334 characters), and 1/3 (The Vault inline snippets); the single AtCoder program does not flip. Non-Vault sources flip in 16/22 (73\%) against 13/28 (46\%) for The Vault, and long Vault classes rarely flip. The logistic regression rests on 29 flip events for three predictors, so we read it as descriptive.
...
DetectCodeGPT's IER is elevated broadly across all three families (16.7--50.0\%, secondary evidence given its weak calibration; Table~\ref{tab:ier} reports the original scoring with independent perturbation seeds), peaking under control-flow rather than formatting, a diffuse pattern with no clear family-specific localization. Its binary accuracy on the test corpus is at chance (99/199 and 100/199 under two perturbation seeds). Re-scoring the 199 unchanged programs under a different seed flips the decision for 37/199 (18.6\%) and moves the score by 0.55 on average (the earlier control on 40 dev-split programs gave 30.0\% and 0.925). Sharing the perturbation seed between $C_0$ and $T(C_0)$ does not rescue the signal: formatting IER is 10/80 (12.5\%) under one seed and 11/82 (13.4\%) under the other, mean $|\Delta|$ is 0.60 and 0.55, no larger than the cross-seed noise floor of 0.55, and the per-pair $\Delta$ does not reproduce across seeds (Spearman $\rho{=}0.05$, $p{=}0.54$; sign agreement 54\%). The language split is also unstable (Python 19.6\% then 8.5\%; Java 2.9\% then 20.0\%). DetectCodeGPT's instability therefore cannot be distinguished from its own perturbation noise.

Human-written and AI-class programs are independent samples, so we use Fisher's exact test rather than a paired test. Both contrasts are far from chance: 29/50 against 2/105 under the same transformation (odds ratio 71, $p{=}9.6{\times}10^{-16}$), and 29/50 against 6/119 under the reverse perturbation (odds ratio 26, $p{=}1.2{\times}10^{-13}$).

\par\addvspace{4pt}\noindent\colorbox{shadecolor}{\parbox{\dimexpr\columnwidth-2\fboxsep\relax}{%
\textbf{Answer to RQ1/RQ2:} DroidDetect-Base's formatting instability is real, class-specific, and directionally asymmetric (58.0\% vs.\ 2/105 under the same transformation, Fisher's exact $p{<}10^{-14}$; 5.0\% under a reverse perturbation). It is concentrated in short programs but holds at matched length, and edit size alone does not explain it. The other two detectors do not yield interpretable stability estimates on this corpus: LLMSniffer's low IER reflects accuracy at or below chance, and DetectCodeGPT performs at chance on this corpus (49.7--50.3\% accuracy) with a high IER (16.7--50.0\%) that is indistinguishable from re-scoring noise, even with shared perturbation seeds. A low IER is therefore not evidence of robustness without a baseline-accuracy check and a noise-floor control.
}}\par\addvspace{4pt}
...
Second, running an intensity-matched version of the reverse messify experiment (Section~4.1), calibrating the messiness transformation's normalized edit size to match formatting's on human code as closely as possible, so that the 58.0\%/5.2\% asymmetry can be reported as a properly matched effect-size comparison rather than only a directional one; alongside this, extending the shared-seed DetectCodeGPT analysis to the other transformation families; and adding at least one further working detector, for example a model fine-tuned on Droid's training split, so that the empirical finding no longer rests on a single detector.
