...
def load_data(file_path):
    data_renamed = sio.loadmat(file_path)
    return data_renamed['T1'], data_renamed['T2'], data_renamed['frequency_offsets'], data_renamed['proton_densities']
...
def bloch_simulation(T1, T2, frequency_offsets, proton_densities, results, num_samples):
    ...
    if idx < num_samples:
        ...
        results[idx] = (T1[idx] * proton_densities[idx]) / T2[idx] * np.exp(1j * frequency_offsets[idx])
...
def simulate_bloch(T1, T2, frequency_offsets, proton_densities):
    ...
    # Launch the kernel
    bloch_simulation[blockspergrid, threadsperblock](d_T1, d_T2, d_frequency_offsets, d_proton_densities, d_results, num_samples)
