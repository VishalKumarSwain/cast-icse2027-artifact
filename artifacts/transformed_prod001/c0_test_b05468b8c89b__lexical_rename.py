def deps_install(app, ref_or_path, install_folder, base_folder, graph_info, remotes=None,
    ...
    if profile_build:
        ...
        out.writeln(profile_host.dumps())
    deps_graph_renamed = graph_manager.load_graph(ref_or_path, create_reference, graph_info, build_modes,
                                          False, update, remotes, recorder,
                                          lockfile_node_id=lockfile_node_id,
                                          is_build_require=is_build_require,
                                          require_overrides=require_overrides)
    graph_lock = graph_info.graph_lock  
    root_node = deps_graph_renamed.root
    ...
    print_graph(deps_graph_renamed, out)
    try:
    ...
    installer.install(deps_graph_renamed, remotes, build_modes, update, profile_host, profile_build,
    ...
    if manifest_folder:
        ...
        for node in deps_graph_renamed.nodes:
            if node.recipe in (RECIPE_CONSUMER, RECIPE_VIRTUAL):
                continue
            retrieve_exports_sources(remote_manager, cache, node.conanfile, node.ref, remotes)
        manifest_manager.check_graph(deps_graph_renamed, verify=manifest_verify,
    ...
    if install_folder:
        ...
        if not create_reference and isinstance(ref_or_path, ConanFileReference):
            neighbours = deps_graph_renamed.root.neighbors()
