def print_connection_dict_advanced(d: dict, file_name: str):
    output_array_renamed = np.zeros((len(d), len(d)))
    i = 0
    ...
    for key in key_list:
        ...
        for name_count_pair in list_of_name_count_pairs:
            ...
            output_array_renamed[i][j] = count
        i = i + 1
    np.savetxt(file_name, output_array_renamed, delimiter=",", fmt="%.0f")
    print(output_array_renamed)
    print("")
    print(key_list)
    print("")
    print("")
