...
class MyGeometeryClass():
        ...
        def reproject_spatial_reference( self, spatial_reference, transformation_name=None):
                spatial_ref_os_renamed = osr.SpatialReference()
                if isinstance(spatial_reference, CRS) or isinstance(spatial_reference, str):
                        ...
                        if isinstance(spatial_reference, str):
                               spatial_ref_os_renamed.ImportFromProj4(spatial_reference)
                        else:
                               spatial_ref_os_renamed.ImportFromEPSG(spatial_reference.to_epsg())

                               self.set_geometry(spatial_ref_os_renamed.ExportToWkt())
                               return True
