...
def generate_lucky_numbers(limit):
    ...
    lucky_numbers_renamed = []
    for j in range(1, limit):
        if is_lucky_number(j):
            lucky_numbers_renamed.append(j)
    return lucky_numbers_renamed

def main():
    # Generate all lucky numbers up to 1000
    lucky_numbers_renamed = generate_lucky_numbers(1000)
    ...
    if is_lucky_number(int(a)):
        ...
        if any(int(a) % lucky == 0 for lucky in lucky_numbers_renamed):
            print('YES')
        else:
            print('NO')
