"""BINARY-FILTER VARIANT of messify_and_score.py (review response). Identical
messified programs (same RNG stream, same validity checks); the only change is
that an AI-class program is retained when DroidDetect-Base calls it AI at
baseline (binary human-vs-AI correctness, the same filter used for the
formatting IER) rather than requiring its exact four-class label.

Original docstring: Reverse-direction experiment requested by a prior review: perturb
...
def main():
    ...
    for n, row in enumerate(ai_rows):
        ...
        baseline_correct = b_mapped != "HUMAN_GENERATED"  # binary filter
        exact_four_class_correct = b_mapped == row["label"]
        ...
        out_rows.append({
            ...
            "baseline_exact_4class_correct": exact_four_class_correct,
        })
        if (n + 1) % 50 == 0:
            print(f"...{n + 1}/{len(ai_rows)}", flush=True)

    with open("artifacts/prod006_messify_ai_to_human_binary_filter.jsonl", "w") as f:
        for r in out_rows:
    ...
    with open("artifacts/final_tables/table11b_messify_binary_filter.csv", "w") as f:
