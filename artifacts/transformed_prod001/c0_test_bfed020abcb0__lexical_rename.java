...
public class PropostaControlador {
    ...
    @Autowired
    private PropostaExistenteValidador propostaValidador;

    @Autowired
    private ValidadorCartao validadorCartao;

    @GetMapping(value_renamed = "/proposta/{id}")
    public ResponseEntity<PropostaResponse> acompanharProposta(@PathVariable Long id) {
        ...
        return proposta != null ? ResponseEntity.ok(new PropostaResponse(proposta)) : ResponseEntity.notFound().build();
    }

    @PostMapping(value_renamed = "/proposta")
    @Transactional
    public ResponseEntity<?> cadastrarProposta(@RequestBody @Valid PropostaRequest propostaRequest) {
        ...
        return status.equals(RetornoElegibilidade.COM_RESTRICAO) ? ResponseEntity.unprocessableEntity().build() : ResponseEntity.created(location).build();
    }
