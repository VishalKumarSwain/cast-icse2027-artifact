...
public class PerformanceMonitor {
    ...
    private String logFilePath_renamed;

    public PerformanceMonitor(String logFilePath_renamed) {
        this.logFilePath_renamed = logFilePath_renamed;
        this.successCount = 0;
        this.errorCount = 0;
    }
    ...
    private void logPerformance(boolean success) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(logFilePath_renamed, true))) {
            String status = success ? "SUCCESS" : "ERROR";
            long executionTime = endTime - startTime;
            String logEntry = String.format("%s | Job Status: %s | Execution Time: %d ms | Total Successful: %d | Total Errors: %d%n",
