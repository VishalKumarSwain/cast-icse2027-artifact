public class DefaultFileLockContentionHandler implements FileLockContentionHandler, Stoppable {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(DefaultFileLockContentionHandler.class);
  ...
  public DefaultFileLockContentionHandler(
      ExecutorFactory executorFactory, InetAddressFactory addressFactory) {
    ...
    return new Runnable() {
      public void run() {
        try {
          ...
          // Logging exception here is only needed because by default Gradle does not show the stack
          // trace
  ...
  private void acceptConfirmationAsLockRequester(long lockId, int port) {
    ...
    LOGGER.debug(
        "Gradle process at port {} confirmed unlock request for lock with id {}.", port, lockId);
  }
  ...
  public boolean maybePingOwner(int port, long lockId, String displayName, long timeElapsed) {
    ...
    if (Integer.valueOf(port).equals(unlocksRequestedFrom.get(lockId))
        && timeElapsed < PING_DELAY) {
